...
for i=1:Data.nalpha
 ...
 % dualpha(i)= (-1/2*(ylabel-mu)'*dinvQlalpha*(ylabel-mu) + (dbl' - mu'*dinvQlalpha)*(ylabel-mu) + 1/2*computeTraceParallel(invQL,dinvQlalpha))*exp(ualpha(i));%-2*exp(ualpha(i));
   ...
   dualpha(i)= (-1/2*(ylabel-mu)'*dinvQlalpha*(ylabel-mu) + (dbl' - mu'*dinvQlalpha)*(ylabel-mu) + 1/2 * blokTraceInvMatrix(invQL, dinvQlalpha,nn,Data.Ttr)) * exp(ualpha(i));%-2*exp(ualpha(i));
